...
def get_data_mel(dataset, outdir, is_del, dur):
   ...
   for nFile in range(0, len(file_name_list)):
       ...
       for i in range(0, int(split_num)):
           ...
           wav_res = librosa.core.resample(wav_split, org_fs, res_fs)
           wav_res = wav_res + eps

           # get mfcc
           mfcc = librosa.feature.mfcc(wav_res,
                                       res_fs,
                                       n_mfcc=n_bin
                                      )
           ## scale [0:1]
           ...
           ## reshape
           [nFreq, nTime] = np.shape(mfcc)
           mfcc = np.reshape(mfcc, (1, nFreq, nTime, 1))
           if nFreq != n_bin or nTime !=313:
               print('---------------- ERROR: MISMATCH DIMISION: {}, {}'.format(nFreq, nTime))
               exit()
           #print(np.shape(mfcc))
           #exit()
           #if is_del == 'yes':
           #    mel_delta1 = deltas(mel1)
           #    mel_delta_delta1 = deltas(mel_delta1)
           #    mel_spec = np.concatenate((mel1[:,:,4:-4,:],mel_delta1[:,:,2:-2,:], mel_delta_delta1), axis=-1)
           #else:
           #    mel_spec = mel1
           ...
           if not os.path.exists(des_dir):
              os.makedirs(des_dir)
           file_des = os.path.join(des_dir, file_name + '_' + str(i) + label_dict[file_name])
